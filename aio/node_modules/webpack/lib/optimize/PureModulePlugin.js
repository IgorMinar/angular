...
const HarmonyImportSpecifierDependency = require("../dependencies/HarmonyImportSpecifierDependency");
  ...
  console.log("LOADING PURE MODULE PLUGIN");
class PureModulePlugin {

	apply(compiler) {
  console.log("APPLYING PURE MODULE PLUGIN");
		...
		compiler.plugin("compilation", compilation => {
			compilation.plugin("optimize-dependencies", modules => {
				...
				// Capture reexports of pure modules
				for(const module of modules) {
					const removeDependencies = [];
					for(const dep of module.dependencies) {
						if(dep instanceof HarmonyImportSideEffectDependency) {
							if(dep.module && dep.module.pureModule) {
								removeDependencies.push(dep);
							}
						} else if(dep instanceof HarmonyExportImportedSpecifierDependency) {
							if(module.pureModule) {
				...
				// Update imports along the reexports from pure modules
				const updates = [];
