...
export = function traceurCompiler(inputTree, destExtension, destSourceMapExtension, options) {
...
class TraceurCompiler implements BroccoliTree {
  ...
  constructor(public inputTree: BroccoliTree,
              private destExtension: string,
              private destSourceMapExtension: string,
  ...
  rebuild() {
    ...
    diffResult.changedPaths.forEach((changedFilePath) => {
      ...
      if (extension === '.js' || extension === '.es6' || extension === '.cjs') {
        ...
        //console.log('traceur compiling: %s', filepath)
    ...
    diffResult.removedPaths.forEach // todo remove files from cache

    // just symlink the cache and output tree
    fs.rmdirSync(this.outputPath);
    symlinkOrCopy.sync(this.cachePath, this.outputPath);

    console.log(`Traceur transpiled ${filesTranspiled} files`);
  }
